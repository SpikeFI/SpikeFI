...
| 4 | `tier4_training/` | training mode: `run_train`, self-containment, persistent-fault semantics | GPU | 13 | ~2.5 s |
...
pure-Python registry checks still run on a CPU-only machine. The suite currently runs **202 tests**
(154 across the tiers above + 48 in the coverage matrix) in **~5 s** on GPU (measured on a
Quadro RTX 4000).
...
`hand_mutate_neuron_output`), layer-invocation probes for the Tier 3 work-counting tests
(`count_invocations` for a single net; `capture_invocation_widths` for a whole `Campaign.run()` call,
since `campaign.faulty` only exists once `_pre_run()` has built it, recording each invocation's batch
width so an optimization that drops samples is told apart from one that keeps them; and
`count_invocations_during_run`, its how-many-times-only wrapper), and two round-execution
